...
class AirflowService:
    def __init__(self):
        self.dag_directory = Path(os.getenv("AIRFLOW_DAG_DIR", "/opt/airflow/dags/generated"))
        self.dag_directory.mkdir(parents=True, exist_ok=True)

        self.airflow_api_url = os.getenv("AIRFLOW_API_URL", "http://localhost:8080/api/v1")
        self.airflow_auth = (
            os.getenv("AIRFLOW_USERNAME", "admin"),
            os.getenv("AIRFLOW_PASSWORD", "admin"),
        )

    def ensure_dag(self, connection_id: str) -> str:
        dag_id = f"ingest_connection_{connection_id}"
        dag_content = generate_dag(dag_id=dag_id, connection_id=connection_id)
    ...
    def deploy_and_trigger(self, connection_id: str, run_id: str) -> tuple[str, str]:
        dag_id = self.ensure_dag(connection_id)
